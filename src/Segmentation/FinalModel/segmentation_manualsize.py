...
class Segmentation:
    ...
    def Segment(self, pImage, pPaddingRatio=0.0, pDilationSElem=None, pCavityFilling=False, pPredictSize=None, trainedObjSize=40, targetObjSize=20):
        ...
        # use manually provided object size parameter
        resizeTo = (float(trainedObjSize) / float(targetObjSize / 2.0)) * (float(image.shape[0] + image.shape[1]) / 2.0)
        finalSize = int(round(resizeTo / 64) * 64)

        print('trained: %f / target: %f / image_size: %dx%d computed: %f' % (trainedObjSize,targetObjSize,image.shape[0],image.shape[1],finalSize))

        if finalSize < 128:
            finalSize = 128
        elif finalSize > self.__mLastMaxDim:
            finalSize = self.__mLastMaxDim

        print('max: %s / finalSize: %s' % (self.__mLastMaxDim,finalSize))

        #self.__mLastMaxDim = finalSize
        #self.__mLastMaxDim = finalSize
        rebuild = True
        ...
        if rebuild:
            ...
            self.__mConfig.IMAGE_MAX_DIM = finalSize
            self.__mConfig.IMAGE_MIN_DIM = finalSize
...
# manually provide the object size parameter in the .json file
if "target_object_size" in params:
    target_object_size = float(params["target_object_size"])
...
method = Segmentation(pModelPath=modelPath, pConfidence=confidence, pNMSThreshold=nmsThresh, pMaxDetNum=maxDetNum, maxDim=defaultSize)
...
for index, imageFile in enumerate(imageFiles):
    ...
    mask, masks, scores = method.Segment(pImage=image, pPaddingRatio=padding, pCavityFilling=cavityFill, pDilationSElem=dilationStruct, trainedObjSize=40, targetObjSize=target_object_size)
